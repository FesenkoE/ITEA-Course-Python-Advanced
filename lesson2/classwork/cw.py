...
# вкуса. Никаким образом это не влияет ни на производительность кода, ни на то, как он будет выглядеть архитектурно.
...
class Point3Dv2(Point2D):
    ...
    # элементам нашего класса (полям и методам)
    @classmethod
...
# у вас есть необходимость создать "примесной" класс на основе какого-то из базовых без
...
# Магические методы - это интерфейсы, которые реализуются для того, чтобы класс мог поддерживать различные виды операций
...
class Point2Dv2:
    ...
    def __str__(self):
        """User-friendly вывод информации об объекте"""
        return f"Точка с координатами x: {self.coord_x}, y: {self.coord_y}"

    def __repr__(self):
        """Тоже вывод информации об объекте, но для сервисных нужд"""
